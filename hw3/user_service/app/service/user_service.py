...
def authenticate_user(user_in: UserLogin, db: Session) -> str:
    ...
    token_data = {"sub": {"id": str(user.id), "name": str(user.name), "email": str(user.email), "created_assignments_count": str(user.created_assignments_count), "credibility_score": str(user.credibility_score)}}
