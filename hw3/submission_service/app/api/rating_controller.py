...
async def update(rating_id: int, dto: UpdateRatingDTO, user_token: dict = Depends(extract_user_token)):
    return await _service.update(rating_id, dto, user_token["sub"]["id"])

@router.delete("/{rating_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete(rating_id: int, user_token: dict = Depends(extract_user_token)):
