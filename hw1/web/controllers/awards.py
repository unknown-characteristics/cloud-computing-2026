...
class AwardsController(BaseController):
    ...
    def get_awards(self, contestant_id: int, query_dict: dict[str, list[str]]):
        out = []
        with DBConnection() as conn, conn.cursor() as cursor:
            item = get_by_id(contestant.Contestant, {"contestant_id": contestant_id}, cursor)
            if item == None:
                self.simple_response_code(404)
                return

            for row in cursor.execute(f"select rownum as rn, {', '.join(award.Award.get_lowercase_columns())} from {award.Award.get_table_name()} where contestant_id = :contestant_id", {"contestant_id": contestant_id}):
                out += [award.Award().from_full_tuple(row[1:])]
        
        self.simple_response_code(200)
        self.handler.wfile.write(bytes(json.dumps(out, default=lambda x: x.as_dict()), "UTF-8"))
        self.handler.wfile.flush()

    def get_single_award(self, contest_id: int, prize_id: int, contestant_id: int):
        with DBConnection() as conn, conn.cursor() as cursor:
            item = get_by_id(award.Award, {"contest_id": contest_id, "prize_id": prize_id, "contestant_id": contestant_id}, cursor)
