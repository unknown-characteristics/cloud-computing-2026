...
class Contest:
    ...
    def get_lowercase_columns() -> list[str]:
        return ["id", "name", "hint", "difficulty", "solution", "start_time", "end_time", "status"]
    ...
    def from_full_tuple(self, tuple):
        ...
        self.name = tuple[1]
        self.hint = tuple[2]
        self.difficulty = tuple[3]
        self.solution = tuple[4]
        self.start_time = tuple[5]
        self.end_time = tuple[6]
        self.status = tuple[7]

        return self

    def as_dict(self):
        return {"id": self.id, "name": self.name, "hint": self.hint, "difficulty": self.difficulty, "solution": self.solution, "start_time": self.start_time.isoformat(), "end_time": None if self.end_time == None else self.end_time.isoformat(), "status": self.status}