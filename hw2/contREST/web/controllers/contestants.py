...
class ContestantsController(BaseController):
    ...
    def get_all_contestants(self, query_dict: dict[str, list[str]]):
        ...
        with DBConnection() as conn, conn.cursor() as cursor:
            ...
            result = self.prepare_sql_pagination_filtering_or_bad_req(sql, query_dict, allowed_filter_cols=["name", "school"], sort_cols=contestant.Contestant.get_lowercase_columns())
            if result == None:
                return
    ...
    def post_contestant(self):
        ...
        with DBConnection() as conn, conn.cursor() as cursor:
            ...
            params = {"name": req["name"], "school": req["school"], "id": id_var}
            
            try:
                cursor.execute("INSERT INTO CONTESTANTS(name, school) VALUES(:name, :school) RETURNING id INTO :id", params)
                conn.commit()
            except oracledb.DatabaseError as e:
                error = e.args[0]
                result = contestant.Contestant.simplify_integrity_error_message(error.code, error.message)
    ...
    def post_contest_participation(self, id: int):
        ...
        with DBConnection() as conn, conn.cursor() as cursor:
            ...
            params = {"contest_id": req["contest_id"], "contestant_id": id, "answer": req["answer"], "score": req["score"]}
            try:
