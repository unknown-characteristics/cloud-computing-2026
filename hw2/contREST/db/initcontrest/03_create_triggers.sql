...
begin
    if(:new.status = 'ended' and :old.status = 'active') then
        for rec in (select rn, contestant_id from (select rownum as rn, contestant_id from submissions where contest_id = :new.id and score >= 0.5 order by score desc, submission_time asc)) loop
            begin
